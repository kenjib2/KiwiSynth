...
namespace kiwi_synth
{
    ...
    class KiwiMcp23X17
    {
        public:
            ...
            {
                uint8_t iocon = transport.ReadReg(MCPRegister::IOCON);
            ...
            void interrupt(MCPPort port, uint8_t mode)
            {
                ...
                case CHANGE:
                    //interrupt on change
                    transport.WriteReg(intconreg, 0);
                    break;
                case FALLING:
                    //interrupt falling : compared against defval, 0xff
                    transport.WriteReg(intconreg, 0xFF);
                    transport.WriteReg(defvalreg, 0xFF);
                    break;
                case RISING:
                    //interrupt rising : compared against defval, 0x00
                    transport.WriteReg(intconreg, 0xFF);
                    transport.WriteReg(defvalreg, 0x00);
                    break;
                }
            }
            ...
            {
                transport.ReadReg(MCPRegister::INTF_A, portA, portB);
            }

            void disableInterrupt(MCPPort port)
            {
                transport.WriteReg(MCPRegister::GPINTEN_A + port, 0x00);
            }
            ...
            {
                transport.ReadReg(MCPRegister::INTCAP_A, portA, portB);
            }
