...
bool KiwiMidiParser::Parse(uint8_t byte, KiwiMidiEvent* event_out)
{
    ...
    switch(pstate_)
    {
                ...
                if(incoming_message_.type < MessageLast)
                {
                    ...
                    if(incoming_message_.type == SystemCommon)
                    {
                        ...
                        if(incoming_message_.sc_type == SystemExclusive)
                        {
                            ...
                            incoming_message_.finalSysexMessage = false;
                        }
                        //short circuit
        ...
        case ParserSysEx:
            if (incoming_message_.sysex_message_len >= SYSEX_BUFFER_LEN) {
                // Size has reached the SYSEX_BUFFER_LEN. Start a new message.
                incoming_message_.sysex_message_len = 0;
            }
            ...
            if(byte == 0xf7)
            {
                incoming_message_.finalSysexMessage = true;
                pstate_ = ParserEmpty;
                if(event_out != nullptr)
                {
                    *event_out = incoming_message_;
            ...
            else
            {
                ...
                if (incoming_message_.sysex_message_len >= SYSEX_BUFFER_LEN) {
                    // Size has reached the SYSEX_BUFFER_LEN. Return the full message chunk.
                    if(event_out != nullptr)
                    {
                        *event_out = incoming_message_;
                    }
