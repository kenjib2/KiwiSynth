...
{
    numGpioExpansions_ = gpioExpansionConfig->numGpioExpansions;

    memset(bitValues_, 0, 4 * 16 * sizeof(bool));
    for (int i = 0; i < numGpioExpansions_; i++) {
        KiwiMcp23017 mcp;
        ...
        interrupt_.Init(seed::D18, GPIO::Mode::INPUT, GPIO::Pull::PULLDOWN, GPIO::Speed::LOW);
    }
}
...
{
    bool interrupted = interrupt_.Read();
    if (interrupted || forceRead) {
        uint16_t nextValues;

        // Read all mcps and write to pinValues
        nextValues = mcps_.at(0).Read();
        for (int i = 0; i < 16; i++) {
            bitValues_[mcps_.at(0).i2c_address - 0x20][i] = nextValues & (1 << (i - 1));
        }
        controlListener_->controlGpioUpdate(mcps_.at(0).i2c_address); // Check for NULL if we ever might not be using a controlListener

        nextValues = mcps_.at(1).Read();
        for (int i = 0; i < 16; i++) {
            bitValues_[mcps_.at(1).i2c_address - 0x20][i] = nextValues & (1 << (i - 1));
        }
        controlListener_->controlGpioUpdate(mcps_.at(1).i2c_address); // Check for NULL if we ever might not be using a controlListener

        nextValues = mcps_.at(2).Read();
        for (int i = 0; i < 16; i++) {
            bitValues_[mcps_.at(2).i2c_address - 0x20][i] = nextValues & (1 << (i - 1));
        }
        controlListener_->controlGpioUpdate(mcps_.at(2).i2c_address); // Check for NULL if we ever might not be using a controlListener

        nextValues = mcps_.at(3).Read();
        for (int i = 0; i < 16; i++) {
            bitValues_[mcps_.at(3).i2c_address - 0x20][i] = nextValues & (1 << (i - 1));
        }
        controlListener_->controlGpioUpdate(mcps_.at(3).i2c_address); // Check for NULL if we ever might not be using a controlListener
    }
