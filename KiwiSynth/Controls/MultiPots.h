...
class MultiPots : public IControl
{
    private:
        ...
        GPIO a2_;
        GPIO a3_;
        std::vector<Pin> pinsSignal_;
        std::vector<Pin> pinsDirect_;
        DaisySeed* hw_;
        IControlListener* controlListener_ = nullptr;
        float mpValueBuffer_[NUM_MPS][NUM_CHANNELS];
        float directValueBuffer_[NUM_DIRECT_POTS];
    ...
    public:
        ...
        inline void Process()
        {
            ...
            ReadPots();
            currentPot_ = (currentPot_ + 1) & CHANNELS_MOD; // Same as % NUM_CHANNELS
            // We are setting the pin for the *next* call of Process.
            SelectMpChannel(currentPot_);
        }
}; // class MultiPots
