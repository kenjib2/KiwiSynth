...
void Storage::ReadValidationCode(char* buffer) {
    void* readPtr = qspi_.GetData(VALIDATION_BASE_ADDRESS);
...
void Storage::WriteValidationCode() {
    qspi_.Erase(VALIDATION_BASE_ADDRESS, VALIDATION_BASE_ADDRESS + 4096);
    qspi_.Write(VALIDATION_BASE_ADDRESS, VALIDATION_SIZE, (uint8_t*)VALIDATION_CODE);
}
...
void Storage::WritePatchGroup(int bankNumber, int groupNumber, SavedPatch patches[2]) {
    ...
    qspi_.Erase(writeAddr, writeAddr + PATCH_GROUP_SIZE - 1);
    for (int i = 0; i < 2; i++) {
        uint8_t bytes[sizeof(SavedPatch)];
        memcpy(bytes, &(patches[i]), sizeof(SavedPatch));
        qspi_.Write(writeAddr + i * PATCH_SIZE, PATCH_WRITE_SIZE, bytes);
    }
}
...
SavedPatch Storage::LoadPatch(int bankNumber, int patchNumber) {
    ...
    void* readPtr = qspi_.GetData(PATCHES_BASE_ADDRESS + bankNumber * BANK_SIZE + patchNumber * PATCH_SIZE);
